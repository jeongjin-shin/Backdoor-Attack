...
from data.dataset import Dataset, TestDataset, inverse_normalize, OGADataset, OGAtestDataset, OGAbenignattackDataset, apply_OGA
...
def train(**kwargs):
    ...
    poisoned_trainset = OGADataset(
        dataset=trainset,
        poison_func=lambda x: apply_OGA(x, alpha=0.5, trigger_size=(49, 49)),
        poison_rate=0.3
        )
    ...
    poisoned_testset = OGAtestDataset(
        dataset=testset,
        poison_func=lambda x: apply_OGA(x, alpha=0.5, trigger_size=(49, 49)),
        poison_rate=1
        )
    attackbenign_testset = OGAbenignattackDataset(
        dataset=testset,
        poison_func=lambda x: apply_OGA(x, alpha=0.5, trigger_size=(49, 49)),
        poison_rate=1
        )
